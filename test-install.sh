...
if [ -d "/opt/mcp_facebook" ] && [ -f "/opt/mcp_facebook/index.js" ]; then
  echo -e "${verde}Instalação bem-sucedida!${reset}"
  echo -e "${azul}Conteúdo do diretório /opt/mcp_facebook:${reset}"
  ls -la /opt/mcp_facebook
else
  echo -e "${vermelho}Instalação falhou!${reset}"
  echo -e "${azul}Conteúdo do diretório /opt/mcp_facebook (se existir):${reset}"
